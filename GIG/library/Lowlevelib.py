...
class Shader:
    def __init__(self,size,pos,vertex_file_, fragment_file_) -> None:
        self.shader_ = pygame_shaders.Shader(size,size,pos,vertex_file_, fragment_file_,0)
...
def load_block_cheat(
    ...
    left_down_image = sprite_image_.subsurface([block_size_[0]*4, 0, block_size_[0], block_size_[1]])
    right_down_image = sprite_image_.subsurface([block_size_[0]*4, block_size_[1], block_size_[0], block_size_[1]])

    left_up_image = sprite_image_.subsurface([block_size_[0]*4, block_size_[1]*3, block_size_[0], block_size_[1]])
    right_up_image = sprite_image_.subsurface([block_size_[0]*4, block_size_[1]*2, block_size_[0], block_size_[1]])

    left_right_up_image = sprite_image_.subsurface([block_size_[0]*5, 0, block_size_[0], block_size_[1]])
    left_right_down_image = sprite_image_.subsurface([block_size_[0]*5, block_size_[1], block_size_[0], block_size_[1]])
    left_right_down_up_image = sprite_image_.subsurface([block_size_[0]*5, block_size_[1]*2, block_size_[0], block_size_[1]])
...
class Draw:
    ...
    def draw_dashed_lines(
        ...
        for i in range(len(points)):
            ...
            if i+1<len(points):
                print(i)
                Draw.draw_dashed_line(surface, points[i], points[i+1], color, width, dash_size)
